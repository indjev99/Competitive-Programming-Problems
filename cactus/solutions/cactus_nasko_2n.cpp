...
// #define NEXT (//std::cerr << x << "consumes " << (curr < buffer.size() ? buffer[curr] : 2) << std::endl, ((curr >= buffer.size()) ? 1 : buffer[curr ++]))
...
void dfsEncode(
	...
	//std::cerr << "encoding " << x << " " << parent[x] << " " << p << " " << up << " " << buffer.size() << std::endl;
	//std::cerr << " --------- ";
	for (const auto it : g[x]) {
		//std::cerr << it << " ";
	}
	...
	if (up) {
		...
		for (const auto it : g[x]) {
			if (it == p) { continue; }
			if (d[it] != d[x] + 1) { continue; }
			if (d[parent[it]] < d[x]) {
				found = it;
			}
		}
		dfsEncode(found, x, true, buffer, perm);
	}
	...
	for (const auto it : g[x]) {
		if (it == p) { continue; }
		if (d[it] != d[x] + 1) { continue; }
		if (d[parent[it]] < d[x]) { continue; }
		ending = 0;
		if (parent[it] == x) {
			buffer.push_back(0);
			buffer.push_back(1);
			//std::cerr << x << " pushes 001" << std::endl;
			dfsEncode(it, x, true, buffer, perm);
		} else if (switched) {
			buffer.push_back(0);
			//std::cerr << x << " pushes 0" << std::endl;
			dfsEncode(it, x, false, buffer, perm);
		} else {
			buffer.push_back(0);
			buffer.push_back(0);
			//std::cerr << x << " pushes 000" << std::endl;
			switched = true;
			dfsEncode(it, x, false, buffer, perm);
		}
	}
	...
	buffer.push_back(1);
	//std::cerr << x << " pushes 1" << std::endl;
	//std::cerr << "exiting " << x << " " << buffer.size() << std::endl;
	ending ++;
}
