...
{{196590, 65536, 32768, 16384, 8192, 4096, 2048, 1024, 512, 256, 128, 64, 32, 16, 8, 4, 2, 1, 131072, 196608},
{196606, 65536, 98304, 114688, 122880, 126976, 129024, 130048, 130560, 130816, 130944, 131008, 131040, 131056, 131064, 131068, 131070, 131071, 131071, 196607},
{131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072, 131072}};
...
void generate_skeleton()
{
    ...
    for (int i=START_CHECK;i<FRWRD_ADD;++i)
    {
        if (i<FRWRD_ADD-1)
        {
            ...
            instr[1][i]={STAY,false,1,i+1};
        }
        else
        {
            instr[0][i]={HALT,false};
            instr[1][i]={RIGHT,false,1,BACK_CHECK};
        }
    }
    instr[0][BACK_CHECK]={LEFT,false,0,BACK_CHECK};
    instr[1][BACK_CHECK]={LEFT,false,1,BACK_CHECK};
    instr[START][BACK_CHECK]={STAY,false,START,START_CHECK};
    for (int i=START_CHECK;i<FRWRD_ADD;++i)
    {
        instr[START][i]={STAY,false,START,i+1};
    }
    instr[START][FRWRD_ADD]={RIGHT,false,START,FRWRD_ADD};
    instr[0][FRWRD_ADD]={RIGHT,false,1,BACK_ADD};
