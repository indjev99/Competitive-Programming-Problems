...
void generate_code()
{
    ...
    for (int i=-1;i<SYMBOLS;++i)
    {
        for (int j=0;j<STATES && cnt<n;++j)
        {
            if (cnt<n-1)
            {
                if (j<STATES-1) instr[to_symbol(i)][j]={STAY,true,to_symbol(i),j+1};
                else instr[to_symbol(i)][j]={STAY,true,to_symbol(i+1),0};
            }
            else instr[to_symbol(i)][j]={HALT,true};
            ++cnt;
        }
    }
}
