...
std::vector<bool> genHint(const std::vector<int>& a, const std::vector<int>& b, const std::vector<int>& sol)
{
    ...
    int numABits = findNumBits(n);
    int numBBits = findNumBits(m);
    ...
    for (int bucket = 0; bucket < root; ++bucket)
    {
        ...
        if (t == k || aPos[t] >= end) continue;

        encode(aPos[t], numABits, hint);
        encode(bPos[t], numBBits, hint);
    }
...
std::vector<int> solve(const std::vector<int>& a, const std::vector<int>& b, const std::vector<bool>& hint)
{
    ...
    int numBucks = k / (numABits + numBBits);
    std::vector<int> aPosBuck, bPosBuck;

    for (int i = 0; i < numBucks; ++i)
    {
        aPosBuck.push_back(decode(numABits, hint, i * (numABits + numBBits)));
        bPosBuck.push_back(decode(numBBits, hint, i * (numABits + numBBits) + numABits));
    }
