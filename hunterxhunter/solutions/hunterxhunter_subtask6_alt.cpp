...
void solve()
{
    int* init = &buffer[3][n];
    int* best = &buffer[4][n];
    int* prevTotalBest = &buffer[5][n];
    int* totalBest = &buffer[6][n];

    std::fill(init - n, init + n + 1, NEG_INF);
    init[0] = 0;

    std::fill(prevTotalBest - n, prevTotalBest + n + 1, NEG_INF);
    prevTotalBest[0] = 0;
    ...
    for (int i = 0; i < n; ++i)
    {
        if (!people[i].vis)
        {
            ...
            {
                int currBest = NEG_INF;
                int lb = std::max(-n, balance - n);
                int ub = std::min(n, balance + n);
                for (int balance2 = lb; balance2 <= ub; ++balance2)
                {
                    int balance3 = balance - balance2;
                    currBest = std::max(currBest, best[balance2] + prevTotalBest[balance3]);
                }
                totalBest[balance] = currBest;
            }

            std::swap(prevTotalBest, totalBest);
        }
    }

    ans = *std::max_element(prevTotalBest, prevTotalBest + n + 1);
}
