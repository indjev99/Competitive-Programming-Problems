#include <bits/stdc++.h>

const uint32_t MAX_N = 100000 + 10;
const uint32_t MAX_M = 100000 + 10;
const uint32_t MAX_Q = 100000 + 10;
const uint32_t MAX_SUM = 100000;
const uint32_t SQRT = 300;

std::vector<int> st[MAX_M];
std::unordered_map<int, uint64_t> light[MAX_N];
std::unordered_set<uint64_t> heavy[MAX_N];
std::vector<int> set[MAX_N];
int n, m, q;

uint64_t solve(int a, int b) {
	assert(a != b && 1 <= a && a <= n && 1 <= b && b <= n);
	if (a > b) { std::swap(a, b); }
	uint64_t ret = 0;

	if (light[a].find(b) != light[a].end()) {
		ret += light[a][b];
	}

	if (heavy[a].size() < heavy[b].size()) {
		for (const auto it : heavy[a]) {
			if (heavy[b].find(it) != heavy[b].end()) {
				ret += it;
			}
		}
	} else {
		for (const auto it : heavy[b]) {
			if (heavy[a].find(it) != heavy[a].end()) {
				ret += it;
			}
		}
	}

	return ret;
}

signed main() {
	std::ios_base::sync_with_stdio(false);
	std::cin.tie(NULL);

	int64_t sumSets = 0;

	std::cin >> n >> m;
	assert(1 <= n && n <= MAX_N);
	assert(1 <= m && m <= MAX_M);
	for (int i = 1; i <= m; i ++) {
		int k;
		std::cin >> k;

		set[i].resize(k);
		for (int j = 0; j < k; j ++) {
			std::cin >> set[i][j];
			assert(1 <= set[i][j] && set[i][j] <= n);
		}
		std::sort(set[i].begin(), set[i].end());
		for (int j = 0; j < k - 1; j ++) { 
			assert(set[i][j] != set[i][j + 1]); 
		}

		if (k >= SQRT) {
			for (const auto it : set[i]) {
				heavy[it].insert(i);
			}
		} else {
			for (size_t l = 0; l < k; l ++) {
				for (size_t r = l + 1; r < k; r ++) {
					light[set[i][l]][set[i][r]] += i;
				}
			}
		}

		sumSets += set[i].size();
	}

	assert(sumSets <= MAX_SUM);

	std::cin >> q;
	assert(1 <= q && q <= MAX_Q);
	while (q --) {
		int a, b;
		std::cin >> a >> b;
		std::cout << solve(a, b) << std::endl;
	}

	return 0;
}
