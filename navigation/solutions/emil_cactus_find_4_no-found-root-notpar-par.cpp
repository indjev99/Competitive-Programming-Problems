...
std::pair<int, int> navigate(int currColor, std::vector<int> adjColors)
{
    ...
    assert(adjsByCol[DFS_COLOR].size() + adjsByCol[PARSEEK_OR_PAR_COLOR].size() <= 2);

    if (currColor == PARSEEK_OR_PAR_COLOR && (!adjsByCol[PARSEEK_OR_PAR_COLOR].empty() || adjsByCol[DFS_COLOR].size() == 2))
    {
