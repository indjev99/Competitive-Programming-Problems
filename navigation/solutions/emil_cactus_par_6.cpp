...
std::pair<int, int> navigate(int currColor, std::vector<int> adjColors)
{
    ...
    if (currColor >= DFS_FRONT_COLORS && (!adjsByCol[DFS_FRONT_COLORS].empty() || !adjsByCol[DFS_FRONT_COLORS + 1].empty()))
    {
        int child = -1;
        if (!adjsByCol[DFS_FRONT_COLORS].empty())
        {
            child = adjsByCol[DFS_FRONT_COLORS].front();
        }
        else
        {
            child = adjsByCol[DFS_FRONT_COLORS + 1].front();
        }
    ...
    if (currColor == INIT_COLOR)
    {
        ...
        return {DFS_FRONT_COLORS + idx, par};
    }
