...
constexpr int DFS_NOTPAR_COLOR = 5;
...
std::pair<int, int> navigate(int currColor, std::vector<int> adjColors)
{
    ...
    assert(adjsByCol[DFS_COLOR].size() + adjsByCol[DFS_NOTPAR_COLOR].size() + adjsByCol[DFS_PAR_COLOR].size() <= 2);
    ...
    if (currColor == PARSEEK_COLOR)
    {
        ...
        {
            return {PARSEEK_COLOR, adjsByCol[DFS_NOTPAR_COLOR].front()};
        }
