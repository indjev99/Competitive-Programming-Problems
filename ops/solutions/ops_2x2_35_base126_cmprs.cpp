...
const int SPACE_PER_NUM = EXP_NUM_REGS_IN_NUMBER + 1;

void doOrderPair(int regs1, int regs2)
{
    int tmpReg1 = regs1 + EXP_NUM_REGS_IN_NUMBER;
    int tmpReg2 = tmpReg1 + SPACE_PER_NUM;

    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        int idx = EXP_NUM_REGS_IN_NUMBER - i - 1;
        applyOp(regs1 + idx, regs2 + idx, tmpReg1, expCmpTable);
        if (i > 0) applyOp(tmpReg2, tmpReg1, tmpReg1, expCmpCarryTable);
        applyOp(tmpReg1, regs2 + idx, regs1 + idx, expCpy2IfTable);
        applyOp(tmpReg1, regs2 + idx, regs2 + idx, expCpy1IfTable);

        std::swap(tmpReg1, tmpReg2);
    }
}
