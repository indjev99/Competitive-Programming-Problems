...
constexpr int EXP_NUM_REG_VALS = (NUM_REG_VALS - 1) / 2;
...
TableT expAddIfTable = makeTable();
TableT expAddPostTable = makeTable();
...
void initTables()
{
    ...
    for (int val = 0; val < EXP_NUM_REG_VALS; val++)
    {
        ...
        {
            int newVal = (val - val2 + EXP_NUM_REG_VALS) % EXP_NUM_REG_VALS;
            if (val < val2) expCmpTable[val][val2] = newVal + markLt;
            else if (val > val2) expCmpTable[val][val2] = newVal + markGt;
            else expCmpTable[val][val2] = newVal + markEq;
        }
    }

    for (int val = 0; val < 2 * EXP_NUM_REG_VALS + 1; val++)
    {
        for (int val2 = 0; val2 < 2 * EXP_NUM_REG_VALS + 1; val2++)
        {
            int rv2 = val2 % EXP_NUM_REG_VALS;
            int m1 = val / EXP_NUM_REG_VALS * EXP_NUM_REG_VALS;
            int m2 = val2 / EXP_NUM_REG_VALS * EXP_NUM_REG_VALS;

            if (m1 == markLt || m1 == markGt) expCmpCarryTable[val][val2] = rv2 + m1;
            else expCmpCarryTable[val][val2] = rv2 + m2;
        }
    }
    ...
    for (int val = 0; val < 2 * EXP_NUM_REG_VALS + 1; val++)
    {
        for (int val2 = 0; val2 < EXP_NUM_REG_VALS; val2++)
        {
            int rv1 = val % EXP_NUM_REG_VALS;
            int m1 = val / EXP_NUM_REG_VALS * EXP_NUM_REG_VALS;

            bool swap = m1 == markGt;

            expAddIfTable[val][val2] =
                !swap
                ? val2
                : (val2 + rv1) % EXP_NUM_REG_VALS;

            expAddPostTable[val][val2] =
                !swap
                ? (val2 + rv1) % EXP_NUM_REG_VALS
                : (val2 - rv1 + EXP_NUM_REG_VALS) % EXP_NUM_REG_VALS;
        }
    }
}
...
void doExpand(int in, int out, int tmp)
{
    ...
    {
        doDivInPlace(in, out + i, tmp, NUM_REGS_IN_NUMBER);
    }
}
...
void doUnexpand(int in, int out, int tmp)
{
    for (int i = 0; i < NUM_REGS_IN_NUMBER; i++)
    {
        doExpDivInPlace(in, out + i, tmp, EXP_NUM_REGS_IN_NUMBER);
    }
}
...
const int SPACE_PER_NUM = EXP_NUM_REGS_IN_NUMBER;
...
void doOrderPair(int regs1, int regs2)
{
    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        ...
        applyOp(regs1 + idx, regs2 + idx, regs1 + idx, expCmpTable);
    }

    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        int idx = EXP_NUM_REGS_IN_NUMBER - i - 1;
        if (idx > 0) applyOp(regs1 + idx, regs1 + idx - 1, regs1 + idx - 1, expCmpCarryTable);
        applyOp(regs1 + idx, regs2 + idx, regs2 + idx, expAddIfTable);
        applyOp(regs1 + idx, regs2 + idx, regs1 + idx, expAddPostTable);
    }
}
...
void sortNumbers(int maxN)
{
    ...
    for (int i = maxN - 1; i >= 0; i--)
    {
        ...
        int newP = EXP_NUM_REGS_IN_NUMBER + SPACE_PER_NUM + i * SPACE_PER_NUM;

        doMove(oldP, newP);
    }

    doMove(0, EXP_NUM_REGS_IN_NUMBER);

    for (int i = 0; i < maxN; i++)
    {
        int regs = EXP_NUM_REGS_IN_NUMBER + SPACE_PER_NUM + i * SPACE_PER_NUM;
    ...
    {
        int regs1 = i * SPACE_PER_NUM;
        int regs2 = (i + 1) * SPACE_PER_NUM;

        doOrderPair(regs1, regs2);

        for (int j = i; j > 0; j -= 2)
        {
            int regs1 = (j - 2) * SPACE_PER_NUM;
            int regs2 = (j - 1) * SPACE_PER_NUM;
            int regs3 = j * SPACE_PER_NUM;
            int regs4 = (j + 1) * SPACE_PER_NUM;

            doOrderPair(regs1, regs3);
            doOrderPair(regs2, regs4);
            doOrderPair(regs2, regs3);
        }
    }
    ...
    if (maxN % 2 == 1)
    {
        for (int i = maxN - 1; i > 0; i--)
        {
            int regs1 = (i - 1) * SPACE_PER_NUM;
            int regs2 = i * SPACE_PER_NUM;
