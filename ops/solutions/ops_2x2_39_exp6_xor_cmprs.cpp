...
TableT expXorIfTable = makeTable();
TableT expXorPostTable = makeTable();
...
void initTables()
{
    ...
    for (int val = 0; val < EXP_NUM_REG_VALS; val++)
    {
        for (int val2 = 0; val2 < EXP_NUM_REG_VALS; val2++)
        {
            if (val < val2) expCmpTable[val][val2] = (val ^ val2) | markLt;
            else if (val > val2) expCmpTable[val][val2] = (val ^ val2) | markGt;
            else expCmpTable[val][val2] = (val ^ val2);

            expCmpTable[val | markLt][val2] = (val ^ val2) | markLt;
            expCmpTable[val | markGt][val2] = (val ^ val2) | markGt;
        }
    }
    ...
    for (int val = 0; val < EXP_NUM_REG_VALS; val++)
    {
        for (int val2 = 0; val2 < EXP_NUM_REG_VALS; val2++)
        {
            expXorIfTable[val][val2] = val2;
            expXorIfTable[val | markLt][val2] = val2;
            expXorIfTable[val | markGt][val2] = val2 ^ val;

            expXorPostTable[val][val2] = val2 ^ val;
            expXorPostTable[val | markLt][val2] = val2 ^ val;
            expXorPostTable[val | markGt][val2] = val2 ^ val;
        }
    }
}
...
void doOrderPair(int regs1, int regs2)
{
    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        ...
        applyOp(regs1 + idx, regs2 + idx, regs2 + idx, expXorIfTable);
        applyOp(regs1 + idx, regs2 + idx, regs1 + idx, expXorPostTable);
    }
}
...
void sortNumbers(int maxN)
{
    ...
    {
        int regs = SPACE_PER_NUM + i * SPACE_PER_NUM;
