...
void overwriteFakeNumbers()
{
    /// Prepare N in different format
    assert(MAXN <= 500);
    applyOp(0, 0, 2, copyOp);
    applyOp(1, 1, 3, copyOp);

    applyOp(2, 3, 0, op([](int n0, int n1){
        if (n1 > 0) return 255;
        else return n0;
    }));
    applyOp(2, 3, 1, op([](int n0, int n1){
        if (n1 > 0) return n1 * 256 + n0 - 255;
        else return 0;
    }));
    ...
    for (int i = 1; i <= 255; i++)
    {
        ...
        {
            applyOp(0, 8 * i + byte_idx, 8 * i + byte_idx,
                op([&](int n, int v){
                    return n >= i ? v : base256[byte_idx];
                })
            );
        }
    }

    for (int i = 256; i <= MAXN; i++)
    {
        for (int byte_idx = 0; byte_idx < 8; byte_idx++)
        {
            applyOp(1, 8 * i + byte_idx, 8 * i + byte_idx,
                op([&](int n, int v){
                    return n >= i - 255 ? v: base256[byte_idx];
                })
            );
        }
    }
