...
const int TMP_SPACE = 2;

void doOrderPair(int regs1, int regs2, int tmpReg1, int tmpReg2)
{
    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
...
void sortNumbers(int maxN)
{
    ...
    for (int i = maxN - 1; i >= 0; i--)
    {
        int oldP = poses[i];
        int newP = poses[i + 1];
        int tmpPos = nearestTmpPos[i];

        doExpand(oldP, newP, tmpPos);
    }
    ...
    for (int i = maxN; i >= 0; i--)
    {
        for (int j = 0; j < i - 1; j++)
        {
            int regs1 = poses[j + 1];
            int regs2 = poses[j + 2];
            int tmpPos = nearestTmpPos[j + 1];

            doOrderPair(regs1, regs2, tmpPos, tmpPos + 1);
        }
    }
