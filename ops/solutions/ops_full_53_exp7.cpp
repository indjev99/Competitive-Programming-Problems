...
constexpr int EXP_NUM_REG_BITS = NUM_REG_BITS - 1;
...
TableT expMarkGtTable = makeTable();
...
void initTables()
{
    ...
    int mark = 1 << EXP_NUM_REG_BITS;
    for (int val = 0; val < EXP_NUM_REG_VALS; val++)
    {
        for (int cmp : {CMP_EQ, CMP_LT, CMP_GT})
        {
            int newVal = val;
            if (cmp == CMP_GT) newVal |= mark;
            expMarkGtTable[val][cmp] = newVal;
        }
    }
    ...
    for (int val = 0; val < EXP_NUM_REG_VALS; val++)
    {
        for (int val2 = 0; val2 < EXP_NUM_REG_VALS; val2++)
        {
            ...
            expCpy1IfTable[val | mark][val2] = val;

            expCpy2IfTable[val][val2] = val;
            expCpy2IfTable[val | mark][val2] = val2;
        }
    }
}
...
const int SPACE_PER_NUM = 2 * EXP_NUM_REGS_IN_NUMBER + 2;

void doOrderPair(int regs1, int regs2)
{
    int tmpRegs = regs1 + EXP_NUM_REGS_IN_NUMBER;

    int cmpReg = tmpRegs + EXP_NUM_REGS_IN_NUMBER;
    int cmpTmpReg = cmpReg + 1;

    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        int idx = EXP_NUM_REGS_IN_NUMBER - i - 1;
        applyOp(regs1 + idx, regs2 + idx, i == 0 ? cmpReg : cmpTmpReg, cmpTable);
        if (i > 0) applyOp(cmpReg, cmpTmpReg, cmpReg, orTable);
    }

    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        applyOp(regs1 + i, cmpReg, regs1 + i, expMarkGtTable);
    }

    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        applyOp(regs1 + i, regs1 + i, tmpRegs + i, cpyTable);
    }

    for (int i = 0; i < EXP_NUM_REGS_IN_NUMBER; i++)
    {
        applyOp(regs1 + i, regs2 + i, regs1 + i, expCpy2IfTable);
    }
...
void sortNumbers(int maxN)
{
    ...
    for (int i = maxN - 1; i >= 0; i--)
    {
        ...
        int newP = NUM_REGS_IN_NUMBER + i * SPACE_PER_NUM;
    ...
    {
        int regs = NUM_REGS_IN_NUMBER + i * SPACE_PER_NUM;
        int nRegs = regs - NUM_REGS_IN_NUMBER;
    ...
    for (int i = maxN; i >= 0; i--)
    {
        for (int j = 0; j < i - 1; j++)
        {
            int regs1 = NUM_REGS_IN_NUMBER + j * SPACE_PER_NUM;
            int regs2 = NUM_REGS_IN_NUMBER + (j + 1) * SPACE_PER_NUM;
    ...
    for (int i = 0; i < maxN; i++)
    {
        int oldP = NUM_REGS_IN_NUMBER + i * SPACE_PER_NUM;
