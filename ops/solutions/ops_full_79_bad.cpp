...
const int SPACE_PER_NUM = 3 * NUM_REGS_IN_NUMBER + 2;

void doOrderPair(int regs1, int regs2)
{
    int tmpRegs1 = regs1 + NUM_REGS_IN_NUMBER;
    int tmpRegs2 = tmpRegs1 + NUM_REGS_IN_NUMBER;

    int cmpReg = tmpRegs2 + NUM_REGS_IN_NUMBER;
    int cmpTmpReg = cmpReg + 1;

    doCmp(regs1, regs2, cmpReg, cmpTmpReg);

    doCpy(regs1, tmpRegs1);
    doCpy(regs2, tmpRegs2);

    doCpySetIf(regs1, cmpReg, regs1, 0, {CMP_GT});
    doCpySetIf(regs2, cmpReg, regs2, 0, {CMP_EQ, CMP_LT});
    doOr(regs1, regs2, regs1);

    doCpySetIf(tmpRegs1, cmpReg, tmpRegs1, 0, {CMP_EQ, CMP_LT});
    doCpySetIf(tmpRegs2, cmpReg, tmpRegs2, 0, {CMP_GT});
    doOr(tmpRegs1, tmpRegs2, regs2);
}
