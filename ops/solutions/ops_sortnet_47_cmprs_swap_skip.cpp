...
void doOrderPair(int regs1, int regs2)
{
    ...
    while (regs2 - regs1 > MAX_OP_DIST)
    {
        int next = regs2 - SPACE_PER_NUM;

        while (next - regs1 > MAX_OP_DIST && regs2 - (next - SPACE_PER_NUM) <= MAX_OP_DIST)
        {
            next -= SPACE_PER_NUM;
        }

        poses.push_back(regs2);

        doXor(regs2, next, regs2);
        doXor(regs2, next, next);
        doXor(regs2, next, regs2);
        ...
        for (int gap = block; gap >= 1; gap /= 2)
        {
            for (int j = gap % block; j <= maxN - gap - 1; j += 2 * gap)
            {
                for (int i = 0; i <= std::min(gap - 1, maxN - gap - j - 1); i++)
                {
                    ...
                    int regs1 = (i + j) * SPACE_PER_NUM;
                    int regs2 = (i + j + gap) * SPACE_PER_NUM;
