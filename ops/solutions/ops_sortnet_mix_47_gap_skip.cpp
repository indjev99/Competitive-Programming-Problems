...
const int SPACE_PER_NUM = 2 * NUM_REGS_IN_NUMBER;
...
void doOrderPair(int regs1, int regs2)
{
    ...
    while (regs2 + NUM_REGS_IN_NUMBER - 1 - tmpReg > MAX_OP_DIST)
    {
        int next = regs2 / SPACE_PER_NUM * SPACE_PER_NUM - NUM_REGS_IN_NUMBER;

        while (next + NUM_REGS_IN_NUMBER - 1 - tmpReg > MAX_OP_DIST && regs2 - (next - SPACE_PER_NUM) <= MAX_OP_DIST)
        {
            next -= SPACE_PER_NUM;
        }
    ...
    {
        int idx = NUM_REGS_IN_NUMBER - i - 1;
        applyOp(regs1 + idx, regs2 + idx, i == 0 ? cmpReg : tmpReg, cmpTable);
        if (i > 0) applyOp(cmpReg, tmpReg, cmpReg, orTable);

        applyOp(regs1 + idx, regs2 + idx, tmpReg, xorTable);
        applyOp(tmpReg, cmpReg, tmpReg, cpySetIfTable);
        applyOp(regs2 + idx, tmpReg, regs2 + idx, xorTable);
        applyOp(regs1 + idx, tmpReg, regs1 + idx, xorTable);
    }

    while (regs2 < origRegs2)
    {
        int next = std::min(regs2 + SPACE_PER_NUM, origRegs2);

        while (next < origRegs2 && std::min(next + SPACE_PER_NUM, origRegs2) - regs2 <= MAX_OP_DIST)
        {
            next = std::min(next + SPACE_PER_NUM, origRegs2);
        }
...
void sortNumbers(int maxN)
{
    ...
    const int MAX_SORTNET_PREF = 155;

    const int sortNetPref = std::min(MAX_SORTNET_PREF, maxN);
    for (int block = 1; block < sortNetPref; block *= 2)
    {
        for (int gap = block; gap >= 1; gap /= 2)
        {
            for (int j = gap % block; j <= sortNetPref - gap - 1; j += 2 * gap)
            {
                for (int i = 0; i <= std::min(gap - 1, sortNetPref - gap - j - 1); i++)
                {
    ...
    for (int i = sortNetPref; i < maxN; i += 2)
    {
        for (int j = i - 1; j >= -1; j--)
        {
            if (j >= 0)
            {
                int regs1 = SPACE_PER_NUM + j * SPACE_PER_NUM;
                int regs2 = SPACE_PER_NUM + (j + 1) * SPACE_PER_NUM;
            ...
            if (i + 1 < maxN)
            {
                int regs1 = SPACE_PER_NUM + (j + 1) * SPACE_PER_NUM;
                int regs2 = SPACE_PER_NUM + (j + 2) * SPACE_PER_NUM;
