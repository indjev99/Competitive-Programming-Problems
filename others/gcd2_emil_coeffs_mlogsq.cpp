...
std::vector<long long> partialCoeffs[MAX_VAL + 1];
...
void findCoeffs()
{
    ...
    {
        partialCoeffs[i].resize(MAX_VAL / i + 1, 0);

        partialCoeffs[i][1] = 1;
        for (int j = i * 2; j <= MAX_VAL; j += i)
        {
            int ratio = j / i;
            for (int k = 1; k <= MAX_VAL / j; ++k)
            {
                partialCoeffs[i][ratio * k] -= partialCoeffs[j][k];
            }
        }
    }
    ...
    for (int i = 1; i <= MAX_VAL; ++i)
    {
        ...
        {
            coeffs[i * j] += i * partialCoeffs[i][j];
        }
    }
}
