...
// like solveSmallLine but for a directed parallelepiped
double solveSmallArea(double x1, double y1, double x2, double y2, double dx, double dy)
{
    double l = 0;
    double r = 1;
    ...
    for (int i = 0; i < ITERS; ++i)
    {
        ...
        double d1 = solveLine(x1 + m1 * dx, y1 + m1 * dy, x2 + m1 * dx, y2 + m1 * dy);
        double d2 = solveLine(x1 + m2 * dx, y1 + m2 * dy, x2 + m2 * dx, y2 + m2 * dy);

        if (d1 < d2) l = m1;
        else r = m2;
    }

    return solveLine(x1 + l * dx, y1 + l * dy, x2 + l * dx, y2 + l * dy);
...
void solve()
{
    // we won't have time for the full solution anyway
    if (n > 20) SPLITS = 20;
    ...
    ans = std::max(ans, solveSmallArea(0, 0, 0, BORDER, BORDER, 0));
    ans = std::max(ans, solveSmallArea(0, 0, BORDER, 0, 0, BORDER));
}
