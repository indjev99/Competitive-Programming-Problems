...
struct QueryMaker
{
    ...
    int countXorOnes(int query, const std::vector<int>& currPoss)
    {
        int cnt=0;
        for (int i=0; i<currPoss.size(); ++i)
        {
    ...
    void splitByXor(int query, const std::vector<int>& currPoss, std::vector<int>& xorZeros, std::vector<int>& xorOnes)
    {
        xorZeros.resize(0);
        xorOnes.resize(0);
        for (int i=0; i<currPoss.size(); ++i)
    ...
    bool tryQuery(int query, bool opt, int rem, int currCode, const std::vector<int>& currPoss)
    {
        ...
        cnt=countXorOnes(query,currPoss);
        ...
        splitByXor(query,currPoss,xorZeros,xorOnes);
    ...
    void findQueryRem1(int currCode, const std::vector<int>& currPoss)
    {
        int query;
        std::vector<int> xorZeros,xorOnes;
        if (currPoss.size()==1)
        ...
        {
            int diff=currPoss[0]^currPoss[1];
            query=1;
            while (!(query & diff))
            {
                query<<=1;
        ...
        bruteForceQueries(0,(currCode<<1)+0,xorZeros);
        bruteForceQueries(0,(currCode<<1)+1,xorOnes);
        queries[currCode & ones]=query;
    }
    bool bruteForceQueries(int rem, int currCode, const std::vector<int>& currPoss)
    {
        ...
        else if (rem>1)
        {
            for (int i=1; i<1<<batchSize; i=i<<1)
            {
                ...
                {
                    int query=i | j;
                    if (tryQuery(query,false,rem,currCode,currPoss)) return true;
                }
            }
        }
        else
        {
            findQueryRem1(currCode,currPoss);
            return true;
        }
        return false;
    }
    ...
    bool stateDirections(const std::vector<std::pair<int, int>>& toState, int curr, int code)
    {
    ...
    void proccessBatch(const std::vector<std::pair<int, int>>& toState, int& curr)
    {
        ...
        for (int i=0; i<qpb; ++i)
        {
            code=(code<<1) | makeQuery(toState,curr,code);
        }
        stateDirections(toState,curr,code);
        curr+=batchSize;
    }
};
