...
void run(int n)
{
    ...
    while (curr<toState.size())
    {
        ...
        else
        {
            ...
            {
                both=get_xor({toState[curr+1],toState[curr+2]});
                if (both==false)
                {
                    state_direction(toState[curr+1],true);
                    state_direction(toState[curr+2],true);
                    lastDir=true;
                    askAndState(toState[curr]);
                }
                else
                {
                    ...
                    if (lastDir) askAndState(toState[curr]);
                    else state_direction(toState[curr],true);
                    lastDir=!lastDir;
                    state_direction(toState[curr+2],lastDir);
                }
                curr+=3;
            }
        }
