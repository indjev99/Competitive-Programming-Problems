...
std::optional<bool> checkOne(int i, int j, int d)
{
    ...
    if (cacheE[i][1].size() < cacheE[j][0].size())
    {
        for (int s : cacheE[i][1])
        {
            if (cacheE[j][0].size() < cacheE[s][1].size())
            {
                for (int t : cacheE[j][0])
                {
                    if (cache[s][t]) return true;
                }
            }
            else
            {
                for (int t : cacheE[s][1])
                {
                    if (cache[t][j]) return true;
                }
            }
        }
    }
    else
    {
        for (int t : cacheE[j][0])
        {
            if (cacheE[i][1].size() < cacheE[t][0].size())
            {
                for (int s : cacheE[i][1])
                {
                    if (cache[s][t]) return true;
    ...
    if (cacheE[j][1].size() < cacheE[i][0].size())
    {
        for (int s : cacheE[j][1])
        {
            if (cacheE[i][0].size() < cacheE[s][1].size())
            {
                for (int t : cacheE[i][0])
                {
                    if (cache[s][t]) return false;
                }
            }
            else
            {
    ...
    else
    {
        for (int t : cacheE[i][0])
        {
            if (cacheE[j][1].size() < cacheE[t][0].size())
            {
                for (int s : cacheE[j][1])
                {
                    if (cache[s][t]) return false;
