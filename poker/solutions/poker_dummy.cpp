...
void solve()
{
    double thresh = (double) betSize / (ante + betSize);

    for (int i = 0; i < n; ++i)
    {
        openBetProbs[i] = (double) (i + 1) / n >= thresh;
        betCallProbs[i] = (double) (i + 1) / n >= thresh;
        checkBetProbs[i] = (double) (i + 1) / n >= thresh;
        checkBetCallProbs[i] = (double) (i + 1) / n >= thresh;
    }
}
