...
void genTripsUtil(int left, int right)
{
    ...
    while (2 * buck + 2 * finBuck <= numVals)
    {
        finBuck *= 2;
        buck = std::min((numVals - finBuck) / 2, finBuck);
    }

    if (2 * buck + finBuck > numVals) return;
    ...
    for (int i = 0; i < buck; i++)
    {
        for (int j = 0; j < buck; j++)
        {
            ...
            trips.push_back({i + left, j + buck + left, k + 2 * buck + left});
        }
    }

    genTripsUtil(left, left + buck);
    genTripsUtil(left + buck, left + 2 * buck);
    genTripsUtil(left + 2 * buck, right);
}
