...
int findDist(const Graph& graph)
{
    std::vector<int> dist(graph.n, -1);
    std::deque<int> deq;

    dist[graph.start] = 0;
    deq.push_back(graph.start);
    ...
    while (!deq.empty())
    {
        ...
        for (const Edge& edge : graph.edges[curr])
        {
            int nextDist = currDist + edge.len;
            if (dist[edge.to] == -1 || nextDist < dist[edge.to])
            {
                dist[edge.to] = nextDist;
                if (edge.len == 0) deq.push_front(edge.to);
                else deq.push_back(edge.to);
            }
        }
    }
...
Program linkProgram(const InstrStream& instrStream)
{
    ...
    {
        std::cerr << "Tried to allocate too many stacks: " << program.numStacks << std::endl;
        fail();
    }
...
int oneTest(std::ifstream& in, const Program& program)
{
    ...
    int dist = findDist(graph);
    if (result.first != dist)
    {
        std::cerr << "Program returned incorrect answer" << std::endl;
        fail();
    }
...
int main(int argc, char *argv[])
{
    ...
    int maxIters = 0;
    maxIters = std::max(maxIters, oneTest(in, program));
    maxIters = std::max(maxIters, oneTest(in, program));
    maxIters = std::max(maxIters, oneTest(in, program));
    maxIters = std::max(maxIters, oneTest(in, program));

    double score;
    if (program.numStacks <= 2 && maxIters < ITERS_100) score = 1.0;
    else if (program.numStacks <= 2) score = 0.9;
    else if (program.numStacks == 3) score = 0.7;
    else if (program.numStacks == 4) score = 0.5;
    else if (program.numStacks == 5) score = 0.4;
    else score = 0.3;

    std::cerr << "Stacks allocated: " << program.numStacks << std::endl;
    std::cerr << "Maximum iterations: " << (maxIters + 999) / 1000 << "k" << std::endl;
