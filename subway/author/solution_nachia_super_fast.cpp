...
RegularBipartiteGraph BipartiteRegularizeEdgeColorEquivalent(
    int n1, int n2, std::vector<std::pair<int, int>> edges
) {
...
std::vector<int> RegularBipartiteEdgeColor(const RegularBipartiteGraph& g) {
    ...
    int nx_flag = 0;
    auto euler_splitting = [&](std::vector<int> pl,
                               std::vector<int> pr) -> std::vector<int> {
        nx_flag++;
    ...
    auto swap_group = [&](const std::vector<int>& el, std::vector<int>& em,
                          const std::vector<int>& er) -> void {
        for(int i = 0; i < n; i++) {
            int len = std::min(em[i] - el[i], er[i] - em[i]);
            std::swap_ranges(
                inci.begin() + el[i], inci.begin() + (el[i] + len),
                inci.begin() + (er[i] - len)
            );
            em[i] = er[i] + el[i] - em[i];
        }
    };
    auto take_matching = [&](int s, int d) -> void {
        std::vector<int> pl(n);
        std::vector<int> pr(n);
        for(int i = 0; i < n; i++) {
            pl[i] = i * g.k + s;
        ...
        for(int w = 1; w < md; w *= 2) {
            if(alpha & w) {
                ...
                int count_edges = 0;
                for(int i = 0; i < n; i += 2) {
                    count_edges += pm[i] + pl[i] - plm[i] * 2;
                }
                if(count_edges < 0) {
                    swap_group(pl, plm, pm);
                }
                std::swap(pm, plm);
            } else {
                ...
                int count_edges = 0;
                for(int i = 0; i < n; i += 2) {
                    count_edges += pr[i] + pm[i] - pmr[i] * 2;
                }
                if(count_edges < 0) {
                    swap_group(pm, pmr, pr);
                }
                std::swap(pm, pmr);
            }
        }
    };
    auto part_color = [&](auto& rec, int s, int d, int c) -> void {
        if(d <= 1) {
            return;
        }
        if(d % 2 == 1) {
        ...
        rec(rec, s + d / 2, d / 2, d / 2);
        rec(rec, s, x, x);
    };
    part_color(part_color, 0, g.k, g.k);
...
std::vector<int> BipartiteEdgeColor(
    ...
    int m = edges.size();
    auto regularized =
        BipartiteRegularizeEdgeColorEquivalent(n1, n2, std::move(edges));
...
bool assign_roads(int n, int m, vector<int> a, vector<int> b) {
    ...
    vector<int> color = BipartiteEdgeColor(n, n, edges);
    vector<vector<int>> ans(d);
    for(int i = 0; i < m; i++) {
        if(color[i] < 0 || color[i] >= d) {
            return false;
        }
