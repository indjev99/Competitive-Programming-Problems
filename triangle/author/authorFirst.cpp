...
std::vector <int> solve(int n)
{
    ...
    for (int j = v.size() - 1 ; j >= 0 ; --j)
    {
        while (true)
        {
            ...
            while (representTO <= n && isFound[(representTO + 1) / 2] && isFound[(representTO + 1) / 2 + (representTO + 1) % 2])
            {
                representTO++;
                representA[representTO] = posOf[representTO / 2];
                representB[representTO] = posOf[representTO / 2 + (representTO & 1)];
            }
