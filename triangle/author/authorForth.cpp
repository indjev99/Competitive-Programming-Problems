...
std::vector <int> solve(int n)
{
    ...
    for (int i = v.size() - 1 ; i >= 0 ; --i)
    {
        ...
        while (q.size())
        {
            ...
            if (representTO <= n && !ask(top, representA[representTO], representB[representTO])) // shoes оптимизацията?
            {
                q.push(top);
                continue;
            }
            ...
            while (possible_values.size() && possible_values.back() >= representTO)
            {
                possible_values.pop_back();
            }

            int l = 0, r = (int)possible_values.size(), mid; 
            while (l < r - 1)
            {
                mid = l + r >> 1;
                if (!ask(top, representA[possible_values[mid]], representB[possible_values[mid]])) l = mid;
                else r = mid;
            }
            
            posOf[possible_values[l]] = top;
            isFound[possible_values[l]] = 1;
            isPosFound[top] = 1;
            while (isFound[mex])
            {
                mex++;
