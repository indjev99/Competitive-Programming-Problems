...
void solve(int n, int m, int k, const std::vector<std::vector<bool>>& blocked)
{
    ...
    auto solveRow = [&](int x, int minY, int maxY)
    {
        ...
        while (list.size() > 1)
        {
            Iter leftIt = list.end();

            for (Iter it = list.begin(); it != list.end(); it = it->next)
            {
                if (it->next == list.end() || it->val->cost <= it->next->val->cost)
                {
                    leftIt = it;
                    break;
                }
            ...
            if (leftIt->next == list.end() ||
                (leftIt != list.begin() && leftIt->prev->val->cost <= leftIt->next->val->cost))
            {
                leftIt = leftIt->prev;
            }
