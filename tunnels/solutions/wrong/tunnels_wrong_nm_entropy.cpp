...
void solve(int n, int m, int k, const std::vector<std::vector<bool>>& walls)
{
    ...
    auto solveRow = [&](int x, int minY, int maxY)
    {
        ...
        auto mergeBack = [&]()
        {
            ...
            double minCost = std::min(leftChild->cost, rightChild->cost);
            double maxCost = std::max(leftChild->cost, rightChild->cost);

            node->cost = maxCost + std::log(1.0 + std::exp(minCost - maxCost));
