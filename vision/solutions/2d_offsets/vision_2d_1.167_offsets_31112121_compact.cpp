...
std::pair<int, int> getMove2d(std::vector<std::vector<int>> v)
{
    ...
    if (newJ > curr)
    {
        if (offs[0] == -2 && offs[6] == 2) return {6, 5};
        if (offs[6] == -2 && offs[0] == 2) return {0, 5};

        if (offs[0] == -2 && offs[6] == NONE) return {6, 2};
        if (offs[6] == -2 && offs[0] == NONE) return {0, 2};

        if (offs[0] == NONE && offs[6] == 2) return {6, 5};
        if (offs[6] == NONE && offs[0] == 2) return {0, 5};
    }
    else
    {
        if (offs[0] == 2 && offs[6] == -2) return {6, 1};
        if (offs[6] == 2 && offs[0] == -2) return {0, 1};

        if (offs[0] == 2 && offs[6] == NONE) return {6, 4};
        if (offs[6] == 2 && offs[0] == NONE) return {0, 4};

        if (offs[0] == NONE && offs[6] == -2) return {6, 1};
        if (offs[6] == NONE && offs[0] == -2) return {0, 1};
    }
